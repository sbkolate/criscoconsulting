...
class CashEntry(AccountsController):
	def __init__(self, *args, **kwargs):
		super(CashEntry, self).__init__(*args, **kwargs)
		...
		for d in self.get('accounts'):
			if d.reference_type=="Journal Entry":
				...
				if not against_entries:
					frappe.throw(_("Bank Entry {0} does not have account {1} or already matched against other voucher")
					...
					for jvd in against_entries:
						if flt(jvd[dr_or_cr]) > 0:
							valid = True
					if not valid:
						frappe.throw(_("Against Bank Entry {0} does not have any unmatched {1} entry")
