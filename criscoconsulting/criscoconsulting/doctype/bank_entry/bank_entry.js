...
frappe.provide("criscoconsulting.bank_entry");
...
erpnext.accounts.JournalEntry = frappe.ui.form.Controller.extend({
	...
	setup_queries: function() {
		...
		me.frm.set_query("reference_name", "accounts", function(doc, cdt, cdn) {
			...
			// journal entry
			if(jvd.reference_type==="Bank Entry") {
	...
	reference_name: function(doc, cdt, cdn) {
		...
		if(d.reference_name) {
			...
			if (d.reference_type==="Bank Entry" && !flt(d.credit) && !flt(d.debit)) {
				this.get_outstanding('Bank Entry', d.reference_name, doc.company, d);
			}
		}
	},
...
cur_frm.cscript.select_print_heading = function(doc,cdt,cdn){
	if(doc.select_print_heading){
		...
		cur_frm.pformat.print_heading = __("Bank Entry");
}
